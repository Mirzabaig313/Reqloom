...
struct CompareNode { CompareOp op; NodePtr lhs; NodePtr rhs; };
struct LogicNode { LogicOp op; NodePtr lhs; NodePtr rhs; };
struct TruthyNode { NodePtr inner; };
struct ArrayNode { std::vector<NodePtr> items; };
...
struct ParsedPredicate::Node {
    std::variant<LiteralNode, JsonPathNode, CompareNode, LogicNode,
                 TruthyNode, ArrayNode> kind;
};
...
public:
    ...
    [[nodiscard]] std::expected<std::vector<Token>, std::string> tokenize() {
        ...
        while (pos_ < src_.size()) {
            ...
            if (c == '(') { out.push_back({Tok::LParen, {}, start}); ++pos_; }
            else if (c == ')') { out.push_back({Tok::RParen, {}, start}); ++pos_; }
            else if (c == '[') { out.push_back({Tok::LBracket, {}, start}); ++pos_; }
            else if (c == ']') { out.push_back({Tok::RBracket, {}, start}); ++pos_; }
            else if (c == ',') { out.push_back({Tok::Comma, {}, start}); ++pos_; }
            else if (c == '$') { out.push_back({Tok::Dollar, {}, start}); ++pos_; }
            else if (c == '.') { out.push_back({Tok::Dot, {}, start}); ++pos_; }
            else if (c == '=' || c == '!' || c == '<' || c == '>') {
...
private:
    std::expected<Token, std::string> readCompareOp() {
        ...
        if (c == '=' && next == '=') { pos_ += 2; return Token{Tok::Eq, {}, start}; }
        if (c == '!' && next == '=') { pos_ += 2; return Token{Tok::Neq, {}, start}; }
        if (c == '<' && next == '=') { pos_ += 2; return Token{Tok::Le, {}, start}; }
        if (c == '>' && next == '=') { pos_ += 2; return Token{Tok::Ge, {}, start}; }
        if (c == '<') { ++pos_; return Token{Tok::Lt, {}, start}; }
        if (c == '>') { ++pos_; return Token{Tok::Gt, {}, start}; }
        return std::unexpected(std::string{"bad operator at position "} +
                               std::to_string(start));
    }
    ...
    std::expected<Token, std::string> readNumber() {
        ...
        if (!sawDigit) {
            return std::unexpected("malformed number at position " +
                                   std::to_string(start));
        }
        return Token{Tok::Number,
...
private:
    ...
    std::expected<NodePtr, std::string> parseCompare() {
        ...
        switch (opTok) {
            case Tok::Eq:      op = CompareOp::Eq;      break;
            case Tok::Neq:     op = CompareOp::Neq;     break;
            case Tok::Lt:      op = CompareOp::Lt;      break;
            case Tok::Le:      op = CompareOp::Le;      break;
            case Tok::Gt:      op = CompareOp::Gt;      break;
    ...
    std::expected<NodePtr, std::string> parseTerm() {
        ...
        switch (t.kind) {
            ...
            case Tok::False:    consume(); return literal(Json(false));
            case Tok::Null:     consume(); return literal(Json(nullptr));
            case Tok::LBracket: return parseArrayLiteral();
            default:
                return std::unexpected("expected term at position " +
                                       std::to_string(t.pos));
        }
    }
    ...
    std::expected<NodePtr, std::string> parseJsonPath() {
        ...
        while (peek().kind == Tok::Dot || peek().kind == Tok::LBracket) {
            if (peek().kind == Tok::Dot) {
                ...
                node.segments.push_back({JsonPathNode::SegKind::Field,
                                         consume().text, 0});
            } else {
                consume();  // '['
                ...
                if (inner.kind == Tok::Number) {
                    std::size_t idx = 0;
                    auto fc = std::from_chars(inner.text.data(),
                                              inner.text.data() + inner.text.size(),
                                              idx);
                    if (fc.ec != std::errc{}) {
        ...
        if (node.segments.size() == 1 &&
            node.segments[0].kind == JsonPathNode::SegKind::Field &&
...
std::optional<Json> evalTerm(const ParsedPredicate::Node& node,
                             const Json& body,
                             int statusCode) {
    return std::visit([&](const auto& n) -> std::optional<Json> {
...
bool evalNode(const ParsedPredicate::Node& node,
              const Json& body,
              int statusCode) {
...
ParsedPredicate::ParsedPredicate(std::unique_ptr<Node> root)
    : root_(std::move(root)) {}
...
std::expected<ParsedPredicate, ChainApiError>
PredicateEvaluator::parse(std::string_view expression) const {
    ...
    if (!root) {
        return std::unexpected(ChainApiError{
            ErrorCode::SchemaInvalid,
            ErrorClass::Schema,
            "predicate: " + root.error()});
    }
    return ParsedPredicate{std::move(*root)};
}

PredicateValue
PredicateEvaluator::evaluate(const ParsedPredicate& predicate,
    ...
    try {
        ...
        return evalNode(*predicate.root_, body, statusCode)
            ? PredicateValue::True
...
std::expected<PredicateValue, ChainApiError>
PredicateEvaluator::eval(std::string_view expression,
                         std::string_view jsonBody,
