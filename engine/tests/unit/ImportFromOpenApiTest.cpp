...
public:
    ScratchDir() {
        const auto unique = "chainapi-openapi-import-" +
...
TEST(ImportFromOpenApi, surfaces_yaml_parse_errors_with_YamlParse_code) {
    ...
    const auto spec = scratch.write("broken.yaml",
                                    "openapi: 3.0.3\ninfo:\n\ttitle: tabbed\n");
...
paths:
  ...
  /pets/{petId}:
    ...
    EXPECT_EQ(project.environments.at("default").at("baseUrl"),
...
paths:
  /widgets:
    ...
    const auto& op = outcome->project.resources.at(ce::ResourceId{"widget"})
                         .operations.at("list");
    ASSERT_TRUE(op.provenance.has_value());
    EXPECT_EQ(op.provenance->source,
              ce::Provenance::Source::OpenApiImport);
    EXPECT_EQ(op.provenance->verifiedAgainst,
              ce::Provenance::VerifiedAgainst::None);
...
paths:
  /orders/{orderId}/items/{itemId}:
    ...
    EXPECT_NE(outcome->warnings.find("path parameter `orderId`"),
              std::string::npos);
    EXPECT_NE(outcome->warnings.find("path parameter `itemId`"),
              std::string::npos);

    const auto& op = outcome->project.resources.at(ce::ResourceId{"item"})
                         .operations.at("get");
    EXPECT_EQ(op.pathTemplate,
              "/orders/{{item.orderId}}/items/{{item.itemId}}");
...
paths:
  /health:
    ...
    const auto& op = outcome->project.resources.at(ce::ResourceId{"health"})
                         .operations.at("list");
...
paths:
  /jobs:
    ...
    auto outcome = ce::importFromOpenApi(spec);
    ASSERT_TRUE(outcome.has_value());
    const auto& op = outcome->project.resources.at(ce::ResourceId{"job"})
                         .operations.at("create");
...
paths:
  /accounts/{id}:
    ...
    auto outcome = ce::importFromOpenApi(spec);
    ASSERT_TRUE(outcome.has_value()) << outcome.error().detail;
    const auto& op = outcome->project.resources.at(ce::ResourceId{"account"})
                         .operations.at("get");
...
paths:
  /pets/{id}:
    ...
    auto outcome = ce::importFromOpenApi(spec);
    ASSERT_TRUE(outcome.has_value()) << outcome.error().detail;
    const auto& op = outcome->project.resources.at(ce::ResourceId{"pet"})
                         .operations.at("get");

    ASSERT_TRUE(op.provenance.has_value());
    EXPECT_EQ(op.provenance->verifiedAgainst,
              ce::Provenance::VerifiedAgainst::OpenApiExample);
    EXPECT_NE(op.provenance->evidence.at("extract.id").substr(0, 9),
              "no_match:");
    EXPECT_EQ(op.provenance->evidence.at("extract.id").substr(0, 9),
              "verified:");
    EXPECT_EQ(op.provenance->evidence.at("extract.name").substr(0, 9),
              "verified:");
}
...
paths:
  /pets/{id}:
    ...
    auto outcome = ce::importFromOpenApi(spec);
    ASSERT_TRUE(outcome.has_value()) << outcome.error().detail;
    EXPECT_NE(outcome->warnings.find("no response example"),
              std::string::npos)
