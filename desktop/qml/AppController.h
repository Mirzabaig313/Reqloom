...
public:
    ...
    // ── Saved examples (explorer child rows). Selection/mutation is fully
    //    wired in WS-C once SavedResponseStore is bridged; for now these are
    //    explorer hooks that report the pending state. ───────────────────────
    Q_INVOKABLE void selectExample(const QString& operationId, const QString& exampleName);
    Q_INVOKABLE void renameExample(const QString& operationId, const QString& exampleName);
