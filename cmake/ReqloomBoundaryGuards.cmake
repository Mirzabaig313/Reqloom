...
#
#   reqloom_forbid_dependencies(<target> <forbidden_lib1> ...)
...
set_property(GLOBAL PROPERTY REQLOOM_BOUNDARY_TARGETS "")

function(reqloom_forbid_dependencies target)
    if(NOT TARGET ${target})
        message(FATAL_ERROR
            "reqloom_forbid_dependencies: '${target}' is not a target")
    endif()

    set(_forbidden ${ARGN})
    set_property(TARGET ${target}
        PROPERTY REQLOOM_FORBIDDEN_DEPS "${_forbidden}")

    get_property(_registered GLOBAL PROPERTY REQLOOM_BOUNDARY_TARGETS)
    list(APPEND _registered ${target})
    set_property(GLOBAL PROPERTY REQLOOM_BOUNDARY_TARGETS "${_registered}")
endfunction()


# Recursively gather every link library reachable from `target`.
# Result is appended to the parent-scope variable `${out_var}`.
function(_reqloom_collect_link_deps target out_var)
    if(NOT TARGET ${target})
        return()
    endif()

    get_property(_visited GLOBAL PROPERTY REQLOOM_VISITED_${target})
    if(_visited)
        return()
    endif()
    set_property(GLOBAL PROPERTY REQLOOM_VISITED_${target} TRUE)
    ...
    foreach(_d IN LISTS _deps)
        ...
        if(TARGET ${_d})
            _reqloom_collect_link_deps(${_d} _collected)
        endif()
    endforeach()
...
function(reqloom_enforce_boundary_rules)
    get_property(_targets GLOBAL PROPERTY REQLOOM_BOUNDARY_TARGETS)

    if(NOT _targets)
        message(STATUS "[Reqloom boundary] No targets registered; skipping.")
        return()
    endif()
    ...
    foreach(_target IN LISTS _targets)
        ...
        get_target_property(_forbidden ${_target} REQLOOM_FORBIDDEN_DEPS)
        if(NOT _forbidden)
            continue()
        endif()

        # Reset visited markers per top-level target. The visited markers
        # are global properties named REQLOOM_VISITED_<target>; they are
        # set during _reqloom_collect_link_deps and persist across calls
        # within a single CMake configure pass. Because we only call
        # reqloom_enforce_boundary_rules() once at the end of the root
        # CMakeLists.txt, sharing the markers across sibling top-level
        # targets is safe — each target rooted at a different node walks
        # its own subgraph.
        set(_collected "")
        _reqloom_collect_link_deps(${_target} _collected)
        ...
        foreach(_forbid IN LISTS _forbidden)
            foreach(_dep IN LISTS _collected)
                if(_dep STREQUAL _forbid)
                    message(SEND_ERROR
                        "[Reqloom boundary VIOLATION] target '${_target}' "
    ...
    if(_any_violation)
        message(FATAL_ERROR
            "[Reqloom boundary] One or more architectural violations were "
            "detected. Fix the offending target(s) before continuing.")
    else()
        message(STATUS "[Reqloom boundary] All targets pass dependency rules.")
    endif()
endfunction()
