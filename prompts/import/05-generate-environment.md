...
Input: the Stage 2 plan's "Environment variables required" table. Output: `reqloom/environments/local.yaml`. Use your file-writing tools — do NOT dump YAML inline in chat.
...
```yaml
# reqloom/environments/local.yaml
...
3. **Per-run override variables** — the plan marks these with "per-run override? yes". Set their values to a recognizable placeholder like `<REPLACE_PER_RUN_VIA_--var>`. The user passes real values via `reqloom run X --var key=value`.
...
Write the file to `reqloom/environments/local.yaml`. Reply with a one-line summary: how many variables were generated, and which ones need user attention before the first run.
